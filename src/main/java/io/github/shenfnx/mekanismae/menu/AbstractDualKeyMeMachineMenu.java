...
/** Shared menu for two-input machines whose resources may be items or chemicals. */
public abstract class AbstractDualKeyMeMachineMenu extends AbstractContainerMenu
        implements MeProcessingMachineMenu {
    private static final int DATA_COUNT = 23;
    private static final int REAL_MACHINE_SLOT_COUNT = AbstractDualKeyMeMachineBlockEntity.TIER_SLOT_INDEX + 1;
    ...
    private final AbstractDualKeyMeMachineBlockEntity machine;
    private final ContainerData data;
    private final SimpleContainer processingDisplay = new SimpleContainer(DISPLAY_SLOT_COUNT);

    protected AbstractDualKeyMeMachineMenu(MenuType<?> menuType, int containerId, Inventory inventory,
            BlockPos pos, Class<? extends AbstractDualKeyMeMachineBlockEntity> expectedType,
            String machineName) {
        ...
        for (int index = 0; index < AbstractDualKeyMeMachineBlockEntity.UPGRADE_SLOT_COUNT; index++) {
            int slotIndex = AbstractDualKeyMeMachineBlockEntity.PATTERN_SLOT_COUNT + index;
        ...
        addSlot(new Slot(machine, AbstractDualKeyMeMachineBlockEntity.TIER_SLOT_INDEX, 8, 29) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return machine.canPlaceItem(AbstractDualKeyMeMachineBlockEntity.TIER_SLOT_INDEX, stack);
            }
    ...
    private void refreshDisplay() {
        ...
        processingDisplay.setItem(1, machine.getSecondProcessingInputDisplay());
    ...
    @Override
    public AbstractDualKeyMeMachineBlockEntity chamber() {
        return machine;
    }

    public int firstChemicalRegistryId() {
        return data.get(17);
    }

    public int firstChemicalAmount() {
        return Math.max(0, data.get(18));
    }

    public int secondChemicalRegistryId() {
        return data.get(19);
    }

    public int secondChemicalAmount() {
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (PatternDetailsHelper.isEncodedPattern(moving)) {
            moved = moveItemStackTo(moving, 0, AbstractDualKeyMeMachineBlockEntity.PATTERN_SLOT_COUNT, false);
        } else if (moving.is(ModItems.SPEED_CARD.get()) || moving.is(ModItems.PARALLEL_CARD.get())
                || moving.is(ModItems.ENERGY_CARD.get())) {
            moved = moveItemStackTo(moving, AbstractDualKeyMeMachineBlockEntity.PATTERN_SLOT_COUNT,
                    AbstractDualKeyMeMachineBlockEntity.TIER_SLOT_INDEX, false);
        } else if (moving.getItem() instanceof ItemTierInstaller) {
            moved = moveItemStackTo(moving, AbstractDualKeyMeMachineBlockEntity.TIER_SLOT_INDEX,
