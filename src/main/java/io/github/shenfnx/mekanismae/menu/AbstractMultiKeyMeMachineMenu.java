...
/** Shared menu for variable item/fluid/chemical input and output layouts. */
public abstract class AbstractMultiKeyMeMachineMenu extends AbstractContainerMenu
        implements MeProcessingMachineMenu {
    private static final int REAL_MACHINE_SLOT_COUNT = AbstractMultiKeyMeMachineBlockEntity.TIER_SLOT_INDEX + 1;
    private static final int DISPLAY_SLOT_COUNT = AbstractMultiKeyMeMachineBlockEntity.RESOURCE_DISPLAY_SLOTS;
    private static final int MACHINE_MENU_SLOT_COUNT = REAL_MACHINE_SLOT_COUNT + DISPLAY_SLOT_COUNT;

    private final AbstractMultiKeyMeMachineBlockEntity machine;
    private final ContainerData data;
    private final SimpleContainer processingDisplay = new SimpleContainer(DISPLAY_SLOT_COUNT);

    protected AbstractMultiKeyMeMachineMenu(MenuType<?> menuType, int containerId, Inventory inventory,
            BlockPos pos, Class<? extends AbstractMultiKeyMeMachineBlockEntity> expectedType,
            String machineName) {
        ...
        for (int index = 0; index < AbstractMultiKeyMeMachineBlockEntity.UPGRADE_SLOT_COUNT; index++) {
            int slotIndex = AbstractMultiKeyMeMachineBlockEntity.PATTERN_SLOT_COUNT + index;
            addSlot(new Slot(machine, slotIndex, 262, 9 + index * 18) {
                @Override
                public boolean mayPlace(ItemStack stack) {
        ...
        addSlot(new Slot(machine, AbstractMultiKeyMeMachineBlockEntity.TIER_SLOT_INDEX, 8, 29) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return machine.canPlaceItem(AbstractMultiKeyMeMachineBlockEntity.TIER_SLOT_INDEX, stack);
            }
        ...
        for (int index = 0; index < DISPLAY_SLOT_COUNT; index++) {
            addSlot(readOnlyDisplaySlot(index, displaySlotX(index), displaySlotY(index)));
        }
        ...
        data = inventory.player.level().isClientSide()
                ? new SimpleContainerData(AbstractMultiKeyMeMachineBlockEntity.CONTAINER_DATA_COUNT)
    ...
    private void refreshDisplay() {
        for (int index = 0; index < DISPLAY_SLOT_COUNT; index++) {
            processingDisplay.setItem(index, machine.getProcessingResourceDisplay(index));
        }
    }
    ...
    @Override
    public AbstractMultiKeyMeMachineBlockEntity chamber() {
        return machine;
    }

    public int inputSlotCount() {
        return machine.getInputSlotCount();
    }

    public int outputSlotCount() {
        return machine.getOutputSlotCount();
    }

    public int displaySlotX(int slot) {
        if (slot < AbstractMultiKeyMeMachineBlockEntity.MAX_INPUT_SLOTS) {
            ...
            return switch (inputSlotCount()) {
                case 1 -> 42;
                case 2 -> 31;
                default -> slot == 2 ? 52 : 28;
            };
        }
        int output = slot - AbstractMultiKeyMeMachineBlockEntity.MAX_INPUT_SLOTS;
        return output < outputSlotCount() ? 91 : -100;
    }

    public int displaySlotY(int slot) {
        if (slot < AbstractMultiKeyMeMachineBlockEntity.MAX_INPUT_SLOTS) {
            if (slot >= inputSlotCount()) {
                return -100;
            }
    ...
    public int resourceType(int slot) {
        return resourceData(slot, 0);
    }

    public int resourceRegistryId(int slot) {
        return resourceData(slot, 1);
    }

    public int resourceAmount(int slot) {
        return Math.max(0, resourceData(slot, 2));
    }

    private int resourceData(int slot, int field) {
        if (slot < 0 || slot >= AbstractMultiKeyMeMachineBlockEntity.RESOURCE_DISPLAY_SLOTS) {
            return 0;
        }
        return data.get(17 + slot * 3 + field);
    }
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (PatternDetailsHelper.isEncodedPattern(moving)) {
            moved = moveItemStackTo(moving, 0, AbstractMultiKeyMeMachineBlockEntity.PATTERN_SLOT_COUNT, false);
        } else if (moving.is(ModItems.SPEED_CARD.get()) || moving.is(ModItems.PARALLEL_CARD.get())
                || moving.is(ModItems.ENERGY_CARD.get())) {
            moved = moveItemStackTo(moving, AbstractMultiKeyMeMachineBlockEntity.PATTERN_SLOT_COUNT,
                    AbstractMultiKeyMeMachineBlockEntity.TIER_SLOT_INDEX, false);
        } else if (moving.getItem() instanceof ItemTierInstaller) {
            moved = moveItemStackTo(moving, AbstractMultiKeyMeMachineBlockEntity.TIER_SLOT_INDEX,
