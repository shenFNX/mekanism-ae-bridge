...
- ME 富集仓、ME 粉碎机、ME 通电冶炼炉、ME 冶金灌注机、ME 锇压缩机、ME 净化仓、ME 化学压射仓、ME 合并机、ME 精密锯木机、ME 化学氧化机与 ME 化学结晶器，可被 AE2 网络识别为处理样板提供器。
- 顶部 9 个编码处理样板槽。
...
- 十一台 ME 机器统一使用真正的 32×32 六面机架贴图；AE 在线时点亮接口，能够实际加工时播放各自的正面动画。
...
- 十一台 ME 机器注册为 Mekanism 现有 JEI 配方分类的加工设备；可直接查看各自能处理的配方。
- 十一台 ME 机器均可由对应的 Mekanism 普通机器与 AE2 样板供应器无序合成。
...
可调整是否消耗 AE2 频道、待机 AE 功耗、红石暂停、能量容量与输入速率、每次加工耗能、基础加工时间、最大预投递缓存、0～8 张加速卡/并行卡的完整倍率曲线，以及基础/高级/精英/终极安装器对并行、能量容量、输入速率和任务缓存的四组倍率。十一台机器默认继承统一的 `machines.defaults`；把对应机器的 `use_defaults` 改为 `false` 后，可以单独设置其能量、耗能、加工时间和缓存上限。
