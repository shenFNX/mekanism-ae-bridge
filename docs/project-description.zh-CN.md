...
- 每台机器拥有 27 个处理样板槽，分为三排。
- 机器空闲时，可直接通过 AE2 样板访问终端集中查看和管理样板。
...
- 安装 Applied Flux 后，可插入感应卡，让机器直接使用 ME 网络中存储的 FE。
- 可选兼容闪电科技的过载供电仪，为机器提供无线供能。
- 支持 JEI 配方分类、AE2 JEI Integration 样板填充、Jade 状态查看，以及中英文 GuideME 指南。
- 可配置加工速度、能耗、缓存容量、每周期处理量、卡片倍率和等级倍率。

## 使用方法

把对应的 Mekanism 原版机器和 AE2 样板供应器放入无序合成栏，即可合成 ME 版本机器。将机器接入 AE2 线缆，并单独提供 FE。为该机器支持的配方编码处理样板，将样板放入机器顶部的 27 个槽位之一，然后从 AE2 终端发起合成。

AE2 发配的任务会进入机器内部缓存，实际产物会返回 ME 存储。每个编码样板都有独立的资源账本，即使不同配方使用相同物品或化学品，也不会互相串料。

## 前置与下载

必须安装 Minecraft 1.21.1、NeoForge、Mekanism、Applied Energistics 2、Applied Mekanistics 和 GuideME。JEI、AE2 JEI Integration、Jade、Mekanism Extras、Applied Flux 与闪电科技均为可选兼容模组。
