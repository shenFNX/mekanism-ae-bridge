...
- 构建产物：`E:\mods\ae2fushu\build\libs\mekanismae-0.1.0-alpha.1.jar`。
...
- 首轮严格过滤在 AE2 调用 `getAvailablePatterns()` 发现已安装样板时，同时校验了完整单次配方比例；JEI 编码的处理样板会把展示数量规范化为 `IInput` 的 multiplier，因此合法样板可能在发配前就被隐藏。
- 发现阶段现只要求输入能匹配本机 Mekanism 配方类型、产物物品一致；AE2 真正调用 `pushPattern()` 后，仍以实际 `KeyCounter` 严格校验物品与化学品的整次比例、产物数量和缓存上限。
- 不同机器仍由各自的 `TYPE_COMPRESSING`、`TYPE_PURIFYING`、`TYPE_INJECTING` 或 `TYPE_METALLURGIC_INFUSING` 隔离，不同编码样板的任务账本也未合并。
- 修复后完整离线构建通过，客户端已重新直达原测试存档，JEI/Jade 初始化完成且无相关异常。

合成配方：

- 六台 ME 机器统一使用无序配方：对应的 Mekanism 普通机器加一个 `ae2:pattern_provider`，产出对应的 ME 机器。
- 配方使用 1.21.1 的 `minecraft:crafting_shapeless` 数据格式，可在 JEI 中由成品查看合成方式，也可由普通机器或样板供应器查看用途。

### 15.18 能量显示统一与首套动态机架（2026-08-12）

本轮以 ME 富集仓为第一台外观样机，完成能量单位统一和真正可同步的在线/加工状态：

- Mekanism Jade 顶部能量条改用官方 `ForgeStrictEnergyHandler` 适配 NeoForge FE，避免把 FE 原始数值误当成 Joule 后出现 `2 MFE` 与 GUI `5M FE` 不一致。
- GUI 和本模组 Jade 文本共用 `EnergyFormatter`；一百万及以上使用 `M`，不足一百万使用 `K`，容量与当前能量遵循同一格式。
- ME 富集仓增加水平朝向、AE 在线和工作中三个方块状态维度；在线状态由 AE2 主节点状态变化同步，工作状态只在当前任务有完整输入、输出未堵塞、未被红石暂停、无故障且能量足够时点亮。
- 正式贴图为严格的 32×32 像素资源，而不是概念图模拟的像素风。粗机架保留 3 像素主导轨和 6 像素加强角，但中央陶瓷与功能面扩大到 22×22。
- 正面为富集晶体处理核心，左面为能量组件，右面为散热维护面，背面为 AE 网络接口；在线/离线分别使用明暗灯路，工作正面使用四帧循环脉冲环。
- 四种模型组合覆盖离线空闲、离线工作、在线空闲、在线工作，并按四个水平朝向生成 16 个 blockstate 变体。

验证：

- 14 张正式贴图通过尺寸断言：静态面均为 32×32，两个工作条带均为 32×128，且每个条带的四帧内容互不相同。
- 28 个资源 JSON 与动画元数据以 UTF-8 严格解析通过，`git diff --check` 通过。
- 完整离线构建成功；客户端直达“新的世界”，集成服务器、JEI 和 Jade 正常初始化，日志中没有富集仓模型或机架贴图缺失。

### 15.19 六台机器动态外观公共化（2026-08-12）

在首台 ME 富集仓实机确认后，本轮把同一套机架状态与资源规范覆盖到现有全部六台机器：

- 新增 `AbstractMeMachineBlock`，由物品机与物品+化学品机两条方块基类共同继承；统一提供水平朝向、AE 在线、工作中属性以及放置、旋转、镜像和默认状态，不在每个具体机器类中复制。
- 在线/工作状态同步下沉到 `AbstractMeProcessingBlockEntity`。物品机与化学机分别按自己的活动任务、输入种类、输出阻塞、红石、故障和能量条件判断是否真正工作；排队但无法推进时不会播放动画。
- 六台机器共用左侧能量面、右侧散热维护面、背面 AE 接口、顶部端口和底部机架；五台新增机器仅提供各自正面的离线、在线和工作动画资源。
- ME 粉碎机使用对向破碎颚；ME 冶金灌注机使用隔离的物品/化学品汇合流；ME 锇压缩机使用上下压头；ME 净化仓使用三级滤层与气泡；ME 化学压射仓使用喷嘴、喷流和反应槽。
- `generate_enrichment_chassis_textures.py` 继续确定性生成全部 32×32 位图及动画条带；`generate_machine_visual_resources.py` 生成五台机器的四种模型组合和每台 16 个 blockstate 变体，后续新机器可以沿用同一资源流程。
- OpenCode `deepseek/deepseek-v4-flash` 完成 Java 公共方块基类的机械性迁移并通过离线编译；主代理审查后完成两类处理账本的状态同步、五种正面设计、资源生成与运行时验证。

验证：

- 33 张机架相关位图尺寸通过断言；12 条工作动画均为 32×128，且每条四帧内容互不相同并具有对应 `.mcmeta`。
- 六台机器共 24 个方块模型和 96 个朝向/在线/工作 blockstate 组合均由脚本可重复生成。
- 速度卡临时物品模型从方块图集不存在的 `minecraft:item/clock` 改为有效的 `minecraft:item/amethyst_shard`，清除客户端每次加载时的无关缺图警告；正式升级卡材质仍留待统一设计。

### 15.20 ME 通电冶炼炉（2026-08-12）

本轮在通用物品加工机基础上新增 ME 通电冶炼炉，并针对 Mekanism 冶炼配方的特殊实现补充扩展点：

- 新机器沿用 9 个样板槽、GTNH 式大容量预投递、按编码样板隔离的任务账本、配方亲和、升级卡、能量、返还、GUI 和 NBT 持久化；加入机器枚举后自动获得全局默认值与单机覆盖配置。
- Mekanism 的 `TYPE_SMELTING` 不等同于普通注册配方列表。原版熔炉配方会在 `MekanismRecipeType.SMELTING` 中动态包装成 `BasicSmeltingRecipe`；直接调用 `RecipeManager#getRecipeFor(TYPE_SMELTING, ...)` 会漏掉铁矿、沙子等原版配方。
- 通用物品机增加可覆写的配方解析入口，只有 ME 通电冶炼炉使用 `MekanismRecipeType.SMELTING.getInputCache().findFirstRecipe(...)`；因此样板发现、投递校验与实际加工统一复用 Mekanism 通电冶炼炉的官方配方视图，其他机器行为不变。
- 注册范围包括方块、物品、方块实体、菜单、客户端屏幕、AE2 节点、NeoForge FE、Mekanism Strict Energy、创造栏、JEI `SMELTING` catalyst、Jade、语言、无序合成配方和掉落表。
- 新增 32×32 耐火炉膛正面，离线时炉芯熄灭，在线时点亮接口，实际加工时以四帧循环表现加热元件与橙色热焰；其余五面继续共用公共机架。
- OpenCode `deepseek/deepseek-v4-flash` 只生成方块、方块实体、菜单和屏幕四个薄壳类；主代理逐文件审查，并实现冶炼缓存适配、全套注册、兼容层、资源生成及验证。

验证边界：

- Java 首轮离线编译通过，确认锁定版本的 Mekanism 冶炼缓存、菜单和注册接口有效。
- 完整离线构建与资源处理通过；76 个资源 JSON/动画元数据可解析，37 张 32×32/32×128 位图尺寸正确，14 条工作动画均具有四个不同帧。
- 客户端成功直达现有测试存档，服务端加载 3467 个配方；新机器配置段自动补齐，JEI 和 Jade 完成运行时初始化，日志中没有电炉注册、模型、贴图或旧存档加载异常。
- 仍需在游戏内编码至少一张原版熔炉配方样板（例如铁矿石 → 铁锭），检查 AE2 能发现、预投递并完成加工；构建和客户端加载不能替代该玩法验收。

### 15.21 原版等级安装器与空闲拆机（2026-08-13）

本轮在全部 ME 机器的公共容器层加入 Mekanism 原版等级安装器：

- GUI 左上角新增一个独立等级槽，不占用原有 9 个样板槽或 8 个 ME 升级卡槽；支持基础、高级、精英和终极等级安装器，槽位上限为 1。
- 安装器作为实际物品保留在槽中，可以随时取出并降级；NBT 使用独立的 `TierInstaller` 键，原有 0～16 号槽位不变，旧存档无需迁移。
- 默认等级并行倍率为 `3/6/10/16`，与并行卡倍率相乘；终极安装器加 8 张并行卡的最终并行为 `16 × 16 = 256`。
- 等级不改变加工速度；能量容量、最大输入和任务缓存的默认等级倍率均为 `2/4/8/16`。四组等级曲线都可在 `[upgrades]` 中独立配置。
- GUI 与 Jade 仅显示最终并行值，不解释等级与卡片的乘法来源。等级槽的空槽提示说明可接受的四种安装器和数量上限。
- 机器只有在内部不存在活动、排队或待返加工资源时才能拆除；样板、ME 升级卡和等级安装器不再阻止拆机，并在普通或创造模式下直接掉落。
- 移除安装器或降低配置不会删除已接收任务；低于现有队列前停止接收新任务。能量容量降低时继续沿用现有规则，截断超出新容量的已存能量。
- 256 并行及高倍率配置下的批量耗能改为饱和计算，避免整数乘法溢出。

验证：

- Java 编译与完整离线构建通过；全部 77 个资源 JSON 以 UTF-8 解析通过，`git diff --check` 通过。
- 仍需游戏内逐项检查四种安装器、终极加 8 张并行卡、拆机掉落、忙碌拆机保护和重进存档。

### 15.22 化学品输出链与 ME 化学氧化机/结晶器（2026-08-14）

本轮补齐第一组以化学品作为产物或原料的机器，并把任务账本从固定物品字段扩展为 AE2 原生资源键：

- 新增 `AbstractSingleKeyMeMachineBlockEntity`，统一处理“一个 AE 资源输入 → 一个 AE 资源输出”的样板。输入和输出可分别为 AE2 物品键或 Applied Mekanistics 化学品键，不再为每种资源方向复制调度逻辑。
- 新增 ME 化学氧化机，严格使用 Mekanism `TYPE_OXIDIZING` 的物品到化学品配方；新增 ME 化学结晶器，严格使用 `TYPE_CRYSTALLIZING` 的化学品到物品配方。
- 样板发现和 AE2 实际投递都会验证本机配方类型、输入资源、输出资源及每次操作比例；任务以编码样板本体、输入键、输出键和单次用量共同隔离，只有完全一致的任务才允许聚合。
- GTNH 式预投递、当前配方亲和、并行加工、输出阻塞、暂停接入、返还至 ME 和加工故障保护全部支持物品与化学品；输出通过 AE2 存储服务按原生资源类型事务式插回网络。
- 活动输入、预计产物、待输出和等待队列使用 `AEKey.toTagGeneric` / `AEKey.fromTagGeneric` 持久化，重进存档时重新校验资源类型、整次比例和账本数量，异常数据进入可返还的安全故障状态。
- 两台机器接入公共 9 样板槽、8 升级槽、等级安装器、FE/Mekanism 能量、GUI、动态机架、配置、合成配方、掉落、创造栏和中英文翻译。
- GUI 的物品槽继续显示物品堆；化学品输入或产物使用对应颜色的样本块与数量提示。Jade 区分物品/化学品输入和待输出，JEI 复用 Mekanism 官方 `OXIDIZING` 与 `CRYSTALLIZING` 分类。

自动与启动验证：

- Java 编译与完整离线构建通过；93 个资源 JSON 可解析，两台机器的 8 张新增正面贴图尺寸和四帧动画条带正确。
- 开发客户端成功进入主菜单；本模组、Jade、JEI、模型和贴图图集均完成加载，没有新增机器的注册、配方或资源异常。
- 仍需在游戏内分别编码“物品 → 化学品”和“化学品 → 物品”样板，检查普通/大倍率预投递、0/1/8 张并行卡、输出空间不足、返还至 ME 和任务中途重进存档。构建和客户端启动不替代这些玩法验收。

实机验收结果（用户于 2026-08-14 确认）：

- ME 化学氧化机和 ME 化学结晶器能够识别处理样板、接收 AE2 发配并完成加工，当前测试未发现问题。

### 15.23 双资源键机器族（2026-08-14）

本轮在单资源键账本基础上新增三台双输入机器：

- 新增 `AbstractDualKeyMeMachineBlockEntity`，活动任务和等待队列都保存编码样板本体、两个 AE 资源键、输出资源键、三组单次用量及剩余操作数。只有样板身份和全部资源字段相同的投递才会聚合，隔离边界仍然是编码样板，不是简单按物品/化学品种类划分。
- ME 反质子核合成机使用 Mekanism `TYPE_NUCLEOSYNTHESIZING`，处理“物品 + 反物质 → 物品”配方；反物质按 JEI 样板中声明的固定单次总量消耗。
- ME 化学溶解室使用 `TYPE_DISSOLUTION`，处理“物品 + 化学品 → 化学品”配方。其 `perTickUsage` 化学品用量固定按 Mekanism 原版基础 200 tick 换算，因此改变本模组加工速度不会改变 JEI 样板要求的每次总量。
- ME 化学灌注器使用 `TYPE_CHEMICAL_INFUSING`，处理“两种化学品 → 化学品”配方；样板身份保留原输入顺序，配方解析同时尝试左右顺序，兼容 Mekanism 可交换的双化学品输入。
- 三台机器复用 GTNH 式大容量预投递、当前样板持续加工、并行倍率、输出阻塞、返还至 ME、NBT 恢复和故障保护；菜单为两个输入和一个输出提供独立只读显示槽。
- 已注册方块、物品、方块实体、菜单、屏幕、AE2 节点、NeoForge FE、Mekanism Strict Energy、机器配置、创造栏、JEI catalyst、Jade、中英文名称、合成配方、掉落表和动态机架资源。

自动验证边界：

- 117 个资源 JSON 以 UTF-8 解析通过；三台机器的静态正面均为 32×32，六条工作动画均为 32×128 且各自包含四个不同帧。
- `clean build --offline --no-daemon --console=plain` 完整构建通过。
- 开发客户端成功进入现有测试存档，服务端加载 3474 个配方，JEI 完成运行时构建，Jade 成功加载本模组插件；日志中没有三台新机器的注册、配方、模型或贴图错误。
- 三台机器仍需分别实测普通/大倍率样板、双样板隔离、0/1/8 张并行卡、返还至 ME、输出堵塞和任务中途重进存档。

### 15.24 可变资源账本与连续化学吞吐（2026-08-14）

本轮完成剩余五种值得纳入的主动单方块处理机：ME 电解分离器、ME 回旋式气液转换机、ME 化学清洗机、ME 营养液化器和 ME 加压反应室。太阳能中子活化器依赖环境条件，公式装配器属于自动合成流程；同位素离心机、颜料类机器和涂装机沿用此前用户确认的不实现决定。

- 新增 `AbstractMultiKeyMeMachineBlockEntity`，统一支持 1～3 路 AE 原生资源输入和最多 2 路实际产物。活动任务与队列保存编码样板本体、每一路资源键、数量、单次比例、全部实际产物和剩余操作数。
- 样板隔离仍以编码样板身份和全部资源比例为边界，不按物品、流体或化学品大类粗略合并。当前样板有完整材料时持续加工，耗尽并返完产物后才切换下一样板。
- 样板可以只声明实际产物的子集。电解分离器的第二气体、营养液化器返还的容器以及加压反应室的可选副产物即使没有写入样板，也会按实际配方生成并独立插回 ME；AE2 只对样板声明的产物进行合成核算。
- ME 电解分离器使用 `TYPE_SEPARATING`，处理“流体 → 两种化学品”；ME 化学清洗机使用 `TYPE_WASHING`，处理“流体 + 化学品 → 化学品”。
- ME 回旋式气液转换机使用 `TYPE_ROTARY`，根据样板输入资源类型自动选择“流体 → 化学品”或“化学品 → 流体”，并同时注册 JEI 冷凝和气化分类。
- ME 营养液化器复用 `TileEntityNutritionalLiquifier.getRecipe(...)` 的官方动态食物配方，不自行复制食物营养值算法；ME 加压反应室使用 `TYPE_REACTION` 处理物品、流体和化学品三输入及可选双产物。
- 每台机器新增独立 `base_operations_per_cycle`。普通物品机默认 1；化学灌注、电解、气液回旋和化学清洗默认 20，再与等级安装器和并行卡倍率相乘。以默认 20 tick 周期计算，1 mB 级连续配方恢复到约每 tick 一份的基础吞吐。
- GUI 为五类工艺分别绘制电解槽、圆形转子、清洗滚筒、食物刀片和压力反应釜，不再共用同一种中央管线图。五台方块正面也使用不同轮廓和四帧工作动画。
- 方块、物品、方块实体、菜单、屏幕、配置、能力、创造栏、JEI、Jade、中英文语言、合成配方、掉落、模型和动态贴图已经注册。

自动验证：

- 157 个资源 JSON 解析通过；五台机器的 10 条工作动画均为 32×128 且各含四个不同帧。
- `build --offline --no-daemon --console=plain` 完整构建通过。
- 客户端通过快速进入参数载入现有测试存档，服务端加载 3479 个配方，JEI 完成运行时构建，Jade 成功加载本模组插件；日志中没有五台新机器的注册、配方、模型、贴图、配置或菜单错误。
- 运行时生成的配置确认化学灌注、电解、气液回旋和化学清洗默认 `base_operations_per_cycle = 20`，其他机器默认 `1`。
- OpenCode `deepseek/deepseek-v4-pro` 对锁定的 Mekanism 10.7.19.85 实际字节码和本轮实现做了只读审查；电解、回旋、清洗、营养液化和加压反应配方的输入顺序、单次用量与实际产物均核对正确。审查提出的“必须声明全部副产物”与既定需求冲突，因此保留样板省略副产物但机器仍实际生成的行为。
- 人工复核发现样板发现阶段原先只检查产物种类，错误数量要到 AE2 实际投递时才拒绝，可能造成合成任务堵塞；现已在向 AE2 发布样板前同时校验输入倍数和已声明产物数量。修正后重新完成 157 个 JSON、10 张静态正面、10 条四帧动画、运行日志和完整离线构建检查。

实机验收与 Beta 发布（用户于 2026-08-17 确认）：
...
### 15.26 GuideME 游戏内指南（2026-08-17）

- 新增静态 GuideME 指南 `mekanismae:guide`，内容目录为 `assets/mekanismae/guide/`，英文页面作为默认基线；中文页面使用 GuideME 21.1.17 的 `_zh_cn/` 前缀规则，缺失语言时由 GuideME 回退到英文。
- 指南包含快速开始、处理样板、GTNH 式预投递与按样板隔离、物品/流体/化学品和副产物、升级与最终并行、GUI 控制、全局/存档配置、19 台机器列表和故障排查共十个主题页。
- 两个公共 GUI 基类都加入 AE2 原生 `OpenGuideButton` 问号按钮；物品机和物品+化学品机的全部 19 个屏幕都可以从右上角打开指南，GuideME 热键仍保持可用。
- 页面中的 `ItemLink`、`ItemImage` 和 `RecipeFor` 使用 GuideME 默认扩展，直接链接到本模组机器、AE2 样板供应器和 Mekanism 等级安装器。

验证：

- 中英文页面各 10 个，前置 YAML、相对链接和 19 个机器资源 ID 均通过本地检查；构建资源目录包含全部 20 个页面。
- `clean build --offline --no-daemon --console=plain` 通过，客户端成功载入主菜单和现有测试存档；GuideME、JEI、Jade、AE2 和本模组初始化无新增错误。
- 测试存档日志在打开指南后出现 GuideME `RecipeCompiler` 与背景精灵加载记录，没有页面解析错误。

### 15.27 自动化任务账本 GameTest（2026-08-17）

- 新增可由 `runGameTestServer` 重复执行的十一项 NeoForge GameTest，直接加载真实 Mekanism 配方、AE2 编码处理样板和本模组方块实体，不使用平行的假账本实现。
- 覆盖升级倍率边界与整数饱和、三类升级卡全局上限、等级安装器、能量和网络开关的 NBT 恢复。
- 覆盖物品任务的 1000 次预投递、两个编码样板隔离、当前样板亲和、队列 NBT 恢复、离线返还不丢资源、损坏账本进入安全故障状态，以及精确缓存上限和超限拒绝。
- 覆盖物品到化学品、双化学品和流体到双化学品三种资源账本；电解样板可省略氧气副产物，但机器仍会记录实际氧气产物。错误的声明产物数量会在接收任务前被拒绝。
- 覆盖忙碌机器拒绝拆除、空闲机器允许拆除并掉落升级卡与等级安装器。
- 覆盖 AE 节点卸载期间不得重新标记区块保存，防止已经拆除的 CPU 空状态覆盖先前持久化的合成任务。
- `runGameTestServer --offline --no-daemon --no-configuration-cache --console=plain` 完成十一项测试，结果全部通过；独立游戏测试服务器正常关闭。

### 15.28 重进存档时保留 AE2 CPU 订单（2026-08-17）

- 从实测存档的区域文件中找到同一区块的连续历史扇区：退出前 AE2 CPU 核心完整保存了 `job`、`waitingFor` 和独立合成链接，机器仍缓存 16 次电解；后一次关服保存保留了机器账本，却把 CPU 核心覆盖为不含 `proxy`、`inventory` 和 `job` 的已拆除状态。
- 根因是公共机器在 AE 节点销毁回调中仍调用 `requestUpdate`、刷新在线/工作方块状态并标记方块实体已更改。这会在同区块 CPU 集群拆除后触发一次晚保存，把关服前已经正确保存的 CPU 任务覆盖掉。
- 公共机器基类现在在 `onChunkUnloaded` 和 `setRemoved` 开始时进入拆除状态；拆除期间以及服务器已经进入停止流程后忽略节点状态回调，`clearRemoved` 后恢复正常处理。这样不依赖同区块内 CPU 与机器的具体卸载顺序，全部机器共享此保护。
- 新增卸载回归 GameTest：清除区块待保存标记后销毁机器 AE 节点，断言回调不会重新把区块标脏。十一项 GameTest 和完整离线构建均通过。

### 15.29 连续流体与化学品机器基础吞吐提升（2026-08-17）
